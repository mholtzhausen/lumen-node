...
pub fn load() -> AppConfig {
    ...
    AppConfig { last_folder, left_pane_pos, right_pane_pos }
}

/// Writes config to `~/.lumen-node/config.yml`, creating the directory if needed.
pub fn save(last_folder: Option<&Path>, left_pane_pos: i32, right_pane_pos: i32) {
    ...
    let content = format!(
        "last_folder: {}\nleft_pane_pos: {}\nright_pane_pos: {}\n",
        folder_str, left_pane_pos, right_pane_pos,
    );
