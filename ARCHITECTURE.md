...
- **`src/config.rs`** — `~/.lumen-node/config.yml` load/save for window, panes, recent folders, **`color_scheme`** (`system` / `light` / `dark`), etc.; **`external_editor`**, full-view favourite HUD keys (`full_view_favourite_icon`, `full_view_favourite_icon_seconds`), and optional startup defaults (`sort_key`, `search_text`, `thumbnail_size`) are loaded but not rewritten by `config::save` (hand-editing only for those defaults). Theme preference is applied through `adw::StyleManager::set_color_scheme` from the header toggle in `src/ui/shell.rs` (and on startup). Single/full view wraps the picture in an overlay for the timed favourite star HUD (`ui::grid::FullViewFavouriteHud`).
