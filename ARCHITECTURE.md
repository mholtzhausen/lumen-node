...
- **`src/icons.rs`** — Registers bundled hicolor symbolics on the GTK icon theme search path for dev, install, and AppImage. All UI chrome uses `lumen-*-symbolic` names under `data/icons/hicolor/scalable/actions/` (favourites, folders, theme, search, trash, etc.), not host FreeDesktop icon names.
