...
- **`src/icons.rs`** — Registers bundled hicolor icons on the GTK icon theme search path for dev, install, and AppImage. Application icon is `com.lumennode.app` under `data/icons/hicolor/scalable/apps/` (and `data/icons/com.lumennode.app.svg` for install/AppImage). All UI chrome uses `lumen-*-symbolic` names under `data/icons/hicolor/scalable/actions/` (favourites, folders, theme, search, trash, etc.), not host FreeDesktop icon names.
- **`src/core/`** — Cross-cutting coordination without owning widgets: `app_state` (startup state from config + folder DB, including `pinned_compare_path` for compare mode, `selected_path` / `selected_paths` mirrors, `batch_list_sort_key`, plus in-memory `prompt_similarity_index` / `similar_paths` / `similar_query_path` / `similar_top_n` for temporary similar-browse filtering), `scan_coordinator` (folder switches, scan generation IDs, coordination with grid defer flags; clears the compare pin and similarity index/filter on folder change).
